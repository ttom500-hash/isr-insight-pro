...
# ==========================================
# 1. המוח הגמיש (כולל ניקוי רווחים אוטומטי)
# ==========================================
def ask_google_direct(prompt):
    ...
    # --- התיקון הקריטי: ניקוי רווחים ---
    # הפקודה הזו מוחקת רווחים בטעות בהתחלה או בסוף
    ...
    return f"שגיאה (בדוק שהמפתח תקין). פרטים: {last_err}"
...
with t2:
    st.subheader("ניתוח דוחות (Clean Key Mode)")
    ...
    if active:
        ...
        if st.button("🚀 הרץ ניתוח") and query:
            with st.spinner("מנקה מפתח API וסורק נתונים..."):
                try:
