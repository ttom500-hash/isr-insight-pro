...
st.markdown(f"""
    ...
    .scroll-v88 {{ display: inline-block; padding-right: 100%; animation: tRunV88 110s linear infinite; font-family: sans-serif; font-size: 0.94rem; color: #ffffff !important; }}
    @keyframes tRunV88 {{ 0% {{ transform: translateX(0); }} 100% {{ transform: translateX(-100%); }} }}
    ...
    <div class="ticker-anchor">
        <div class="m-strip"><div class="scroll-v88">{m_html} &nbsp;&nbsp;&nbsp;&nbsp; | &nbsp;&nbsp;&nbsp;&nbsp; {m_html}</div></div>
        <div class="n-strip"><div class="scroll-v88">📢 מודיעין פיננסי ורגולטורי: {n_html} &nbsp;&nbsp;&nbsp;&nbsp; ● &nbsp;&nbsp;&nbsp;&nbsp; {n_html}</div></div>
    </div>
    """, unsafe_allow_html=True)

# --- 2. מנוע אימות נתונים (Integrity Layer) ---
def validate_data_integrity(extracted_dict):
    ...
    if abs(calc_ratio - extracted_dict['solvency_ratio']) > 1.0:
        reports.append({"status": "error", "msg": f"❌ שגיאת סולבנסי: מחושב {calc_ratio:.1f}% vs דווח {extracted_dict['solvency_ratio']}%"})
    else: reports.append({"status": "success", "msg": "✅ אימות הוני: יחס הסולבנסי תקין."})
    ...
    else: reports.append({"status": "success", "msg": "✅ אימות IFRS 17: פירוט מגזרי תואם למאזן."})
    return reports
...
if not df.empty and d is not None:
    ...
    st.divider()
    tabs = st.tabs(["📉 מגמות ויחסים", "🏛️ סולבנסי II - עומק", "📑 מגזרים IFRS 17", "⛈️ Stress Test - עומק", "🏁 השוואה ענפית - עומק"])
    ...
    with tabs[0]: # 6 יחסים משלימים
        ...
        with r1[0]: render_pro_kpi("Loss Ratio", f"{d['loss_ratio']}%", r"LR = \frac{Claims}{Premium}", "איכות חיתום נטו.", "70%-80%. מעל 85% = כשל חיתומי.", "בחינה של הרעה בטיפול בתביעות.")
        with r1[1]: render_pro_kpi("Expense Ratio", f"{d['expense_ratio']}%", r"ER = \frac{Mgmt \ Exp}{Premium}", "יעילות תפעולית.", "15%-20%. חברות יעילות: 12%-14%.", "עלייה = התנפחות מנגנון הניהול.")
        with r1[2]: render_pro_kpi("שחרור CSM", f"{d['csm_release_rate']}%", r"Release", "קצב הכרת רווח מה-CSM.", "2-2.5% לרבעון.", "קצב מהיר ללא צמיחה שוחק את העתיד.")
        with r2[0]: render_pro_kpi("תשואת השקעות", f"{d['inv_yield']}%", r"Yield", "ביצועי תיק ההשקעות.", "צמוד לריבית + פרמיית סיכון (4-6%).", "פער שלילי מול ריבית ההיוון מסוכן.")
        with r2[1]: render_pro_kpi("הון לנכסים", f"{d['equity_to_assets']}%", r"Ratio", "מינוף וחוסן מאזני.", "8%-12% טווח בטוח.", "יחס נמוך = מינוף גבוה וסיכון ליציבות.")
        with r2[2]: render_pro_kpi("תזרים מפעילות", f"{d['op_cash_flow_ratio']}%", r"CFO/NI", "איכות הרווח - מזומן vs חשבונאות.", "קרוב ל-1.0. מתחת ל-0.7 = אזהרה.", "מעיד על 'רווחי נייר' ובעיות גבייה.")
    ...
    with tabs[1]: # סולבנסי II (Deep-Dive)
        ...
        c1, c2 = st.columns(2)
        with c1:
            rd = pd.DataFrame({'מודול': ['שוק', 'חיתום חיים', 'חיתום בריאות', 'חיתום כללי', 'תפעול'], 'דרישה': [d['mkt_risk'], d['und_risk']*0.4, d['und_risk']*0.3, d['und_risk']*0.3, d['operational_risk']]})
            st.plotly_chart(px.bar(rd, x='דרישה', y='מודול', orientation='h', template="plotly_dark", height=300, color='מודול').update_layout(showlegend=False, paper_bgcolor='rgba(0,0,0,0)'), use_container_width=True)
        with c2:
    ...
    with tabs[3]: # Stress Test (Deep-Dive)
        
        st.write("### ⛈️ סימולטור תרחישי קיצון משולב")
        sc1, sc2, sc3 = st.columns(3)
        with sc1:
            eq_s = st.slider("קריסת מניות (%)", 0, 50, 0, key="st_eq")
            with st.expander("❓ הסבר"): st.write("ירידה בערך הנכסים המוחזקים בתיק המשתתף והנוסטרו.")
        with sc2:
            ir_s = st.slider("שינוי ריבית (bps)", -150, 150, 0, key="st_ir")
            with st.expander("❓ הסבר"): st.write("שינוי בשיעור ההיוון המשפיע על התחייבויות ארוכות טווח (BEL).")
        with sc3:
            cr_s = st.slider("מרווחי אשראי (bps)", 0, 300, 0, key="st_cr")
            with st.expander("❓ הסבר"): st.write("הרחבת מרווחים באג\"ח קונצרני המורידה את ערך תיק האג\"ח.")

        sc4, sc5 = st.columns(2)
        with sc4:
            lp_s = st.slider("ביטולים המוניים (%)", 0, 40, 0, key="st_lp")
            with st.expander("❓ הסבר"): st.write("תרחיש 'ריצה על הקופה' המוביל לאובדן CSM.")
        with sc5:
            mo_s = st.slider("עלייה בתביעות (%)", 0, 30, 0, key="st_mo")
            with st.expander("❓ הסבר"): st.write("הרעה בשיעור התביעות במגזר הבריאות/סיעוד.")
    ...
    with tabs[4]: # Peer Analysis (v87 Depth)
        ...
        fig_s.add_hline(y=q_df['roe'].mean(), line_dash="dash", annotation_text="ממוצע ROE")
        fig_s.add_vline(x=150, line_dash="dot", annotation_text="רף 150%")
        fig_s.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)')
        st.plotly_chart(fig_s, use_container_width=True)

        st.divider()
        # Bar with Average
        met_c = st.selectbox("בחר מדד לדירוג:", ['solvency_ratio', 'roe', 'csm_total', 'combined_ratio', 'expense_ratio'])
        b_df = q_df.sort_values(by=met_c, ascending=False)
        avg_v = b_df[met_c].mean()
        fig_b = px.bar(b_df, x='display_name', y=met_c, color='display_name', text_auto='.1f', template="plotly_dark", height=380)
        fig_b.add_hline(y=avg_v, line_color="white", line_dash="dash", annotation_text=f"ממוצע: {avg_v:.1f}")
        fig_b.update_layout(paper_bgcolor='rgba(0,0,0,0)', showlegend=False)
