...
# --- 1. הנדסת נראות: פתרון Brute-Force CSS ---
...
st.markdown("""
    ...
    /* כפיית טקסט לבן על כל רכיב מרקאדון, פסקה ותווית */
    ...
    /* תיקון תיבות בחירה (Selectbox) - רקע כהה, כתב לבן חובה */
    ...
    /* תיקון POPOVER (חלון הסברים) - מניעת המלבן הלבן והכתב הנעלם */
    ...
    /* כפיית טקסט לבן בתוך חלונית ההסבר לכל האלמנטים */
    div[data-testid="stPopoverBody"] * {
        color: #ffffff !important;
    }
    ...
    /* תיקון גרירת קבצים (File Uploader) - כפיית צבע לבן */
...
with st.sidebar:
    ...
    if not df.empty:
        ...
        sel_name = st.selectbox("בחר חברה לניתוח:", all_comps, key="final_exec_v17")
        c_df = df[df['display_name'] == sel_name].sort_values(by=['year', 'quarter'], ascending=False)
        sel_q = st.selectbox("בחר רבעון דיווח:", c_df['quarter'].unique(), key="final_exec_q_v17")
    ...
    with st.expander("📂 PORTAL: INGEST DATA"):
        st.file_uploader("טען דוח PDF", type=['pdf'], key="uploader_final_v17")

# --- 4. EXECUTIVE DASHBOARD ---
if not df.empty:
    st.title(f"{sel_name} | Executive Control Center")
    st.caption(f"תקופת דיווח: רבעון {sel_q} 2025 | הנתונים מאומתים ✅")
    ...
    params = [
        ...
        ("ROE", f"{d['roe']}%", r"ROE = \frac{Net \ Income}{Equity}", "תשואה להון.", "איכות הניהול והשאת ערך."),
    ...
    with t_trends:
        st.plotly_chart(px.line(c_df, x='quarter', y=['solvency_ratio', 'roe'], markers=True, template="plotly_dark", color_discrete_sequence=['#38bdf8', '#f85149']), use_container_width=True)
        st.write("### 📊 יחסים פיננסיים משלימים")
        r_cols = st.columns(3)
        with r_cols[0]: render_executive_kpi("הון לנכסים", f"{d['equity_to_assets']}%", r"\frac{Eq}{Assets}", "מינוף מאזני.", "איתנות פיננסית.")
        with r_cols[1]: render_executive_kpi("יחס הוצאות", f"{d['expense_ratio']}%", r"\frac{OpEx}{GWP}", "יעילות תפעולית.", "יתרון לגודל.")
        with r_cols[2]: render_executive_kpi("איכות רווח", f"{d['op_cash_flow_ratio']}%", r"\frac{CFO}{NI}", "המרת רווח למזומן.", "נזילות ותזרים.")

    with t_solv:
        
        ca, cb = st.columns(2)
        with ca:
            f_tier = go.Figure(data=[go.Bar(name='Tier 1 (Core)', y=[d['tier1_cap']], marker_color='#38bdf8'), go.Bar(name='Tier 2/3', y=[d['own_funds']-d['tier1_cap']], marker_color='#30363d')])
