...
# --- 1. THE ULTIMATE VISIBILITY SYSTEM (BOARDROOM ELITE) ---
...
st.markdown("""
    ...
    /* כפיית טקסט לבן בוהק על כל רכיב אפשרי */
    ...
    /* תיקון סרגל חיפוש (Selectbox) - רקע כהה וטקסט לבן */
    ...
    /* עיצוב כפתור רענון (Refresh) - Xbox Style */
    ...
    div[data-testid="stMetricValue"] { color: #2dd4bf !important; font-weight: 800 !important; }
    div[data-testid="stMetricLabel"] { color: #94a3b8 !important; }

    /* דגלים אדומים - ניגודיות שיא */
    .critical-banner {
...
with st.sidebar:
    ...
    if not df.empty:
        ...
        sel_name = st.selectbox("בחר ישות פיננסית:", all_comps, key="v12_comp")
        c_df = df[df['display_name'] == sel_name].sort_values(by=['year', 'quarter'], ascending=False)
        sel_q = st.selectbox("תקופת דיווח:", c_df['quarter'].unique(), key="v12_q")
...
if not df.empty:
    ...
    st.caption(f"רבעון {sel_q} לשנת 2025 | הנתונים נטענו אוטומטית ✅")
    ...
    k = st.columns(5)
    params = [
        ("סולבנסי", f"{int(d['solvency_ratio'])}%", r"Ratio = \frac{Own \ Funds}{SCR}", "חוסן הוני רגולטורי.", "יעד 150%."),
    ...
    # ג' : טאבים למחקר עומק
    t_trends, t_solv, t_ifrs, t_stress, t_peer = st.tabs(["📉 מגמות", "🏛️ סולבנסי II", "📑 מגזרים", "⛈️ Stress Engine", "🏁 השוואה"])
