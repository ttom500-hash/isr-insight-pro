...
# --- 1. DESIGN SYSTEM: EXECUTIVE BLUE (MAX VISIBILITY) ---
st.set_page_config(page_title="Apex Executive Command", page_icon="🛡️", layout="wide")

st.markdown("""
    <style>
    /* בסיס המערכת - שחור עמוק וקריא */
    .stApp { background-color: #010409; color: #ffffff !important; }
    ...
    /* תיקון סרגל הצד (Sidebar) */
    section[data-testid="stSidebar"] { background-color: #0d1117; border-left: 1px solid #30363d; }
    section[data-testid="stSidebar"] label { color: #ffffff !important; font-weight: 700 !important; }

    /* תיקון תיבות בחירה (Selectbox) - מניעת כתב לבן על לבן */
    div[data-baseweb="select"] > div {
        background-color: #161b22 !important;
        color: white !important;
        border: 1px solid #3b82f6 !important;
    }
    div[role="listbox"] { background-color: #0d1117 !important; border: 1px solid #3b82f6 !important; }
    div[role="option"] { color: white !important; background-color: #0d1117 !important; }

    /* כרטיסי Metric - כחול עמוק */
    ...
    div[data-testid="stMetricLabel"] { color: #8b949e !important; font-weight: 600; text-transform: uppercase; }

    /* תיקון ה-POPOVER (הסברים) - מניעת לבן על לבן */
...
# פונקציית רנדור אחידה למניעת NameError
def render_executive_card(label, value, formula, explanation, impact):
    st.metric(label, value)
    with st.popover("ℹ️ הסבר מקצועי"):
        st.markdown(f"### {label}")
        st.markdown(f"**ביאור:** {explanation}")
        st.divider()
...
with st.sidebar:
    ...
    if not df.empty:
        sel_name = st.selectbox("בחר חברה:", sorted(df['display_name'].unique()), key="main_comp_v15")
        c_df = df[df['display_name'] == sel_name].sort_values(by=['year', 'quarter'], ascending=False)
        sel_q = st.selectbox("בחר רבעון:", c_df['quarter'].unique(), key="main_q_v15")
    ...
    with st.expander("📂 PORTAL: INGEST DATA"):
        st.file_uploader("גרור דוח PDF", type=['pdf'], key="uploader_v15")
...
if not df.empty:
    ...
    with t_trends:
        st.plotly_chart(px.line(c_df, x='quarter', y=['solvency_ratio', 'roe'], markers=True, template="plotly_dark", color_discrete_sequence=['#3b82f6', '#f85149']), use_container_width=True)
        st.write("### 📊 יחסים פיננסיים משלימים")
        r_cols = st.columns(3)
        with r_cols[0]: render_executive_card("הון לנכסים", f"{d['equity_to_assets']}%", r"\frac{Eq}{Assets}", "מינוף.", "איתנות.")
        with r_cols[1]: render_executive_card("יחס הוצאות", f"{d['expense_ratio']}%", r"\frac{OpEx}{GWP}", "יעילות.", "תפעול.")
        with r_cols[2]: render_executive_card("איכות רווח", f"{d['op_cash_flow_ratio']}%", r"\frac{CFO}{NI}", "נזילות.", "תזרים.")

    with t_solv:
        
        ca, cb = st.columns(2)
        with ca:
            f_tier = go.Figure(data=[go.Bar(name='Tier 1', y=[d['tier1_cap']], marker_color='#3b82f6'), go.Bar(name='Tier 2/3', y=[d['own_funds']-d['tier1_cap']], marker_color='#30363d')])
