...
# --- 1. הגדרות מערכת ונראות (v34 CONSOLIDATED & PROTECTED) ---
st.set_page_config(page_title="Apex Executive Command", page_icon="🛡️", layout="wide")

# פונקציית משיכת נתוני בורסה מהירה
@st.cache_data(ttl=600)
def get_live_ticker():
    t_map = {
        '^TA125.TA': 'ת"א 125', 'ILS=X': 'USD/ILS', '^GSPC': 'S&P 500',
        '^IXIC': 'NASDAQ', 'EURILS=X': 'EUR/ILS', '^TNX': "אג''ח 10ש'"
    }
    try:
        ...
        return " &nbsp;&nbsp;&nbsp;&nbsp; | &nbsp;&nbsp;&nbsp;&nbsp; ".join(parts)
    except: return "טוען נתוני שוק..."

ticker_html = get_live_ticker()

# הזרקת CSS לפתרון ניגודיות, פרופורציות ואייקונים
st.markdown(f"""
    <style>
...
# --- 2. BACKEND & PROTECTED COMPONENTS ---
...
def render_metric_card(label, value, formula, desc, imp):
    st.metric(label, value)
    # מנגנון הגנה לשגיאת ה-AttributeError שקיבלת
    try:
        with st.popover("ℹ️ ניתוח"):
            st.markdown(f"#### {label}")
            st.write(desc); st.divider(); st.latex(formula); st.info(f"**דגש:** {imp}")
    except AttributeError:
        with st.expander("ℹ️ ניתוח"):
            st.markdown(f"#### {label}")
            st.write(desc); st.latex(formula); st.info(f"**דגש:** {imp}")
...
with st.sidebar:
    ...
    if not df.empty:
        sel_name = st.selectbox("בחר חברה:", sorted(df['display_name'].unique()), key="s_v34")
        c_df = df[df['display_name'] == sel_name].sort_values(by=['year', 'quarter'], ascending=False)
        sel_q = st.selectbox("בחר רבעון:", c_df['quarter'].unique(), key="q_v34")
        d = c_df[c_df['quarter'] == sel_q].iloc[0]
        if st.button("🔄 רענן נתונים"): st.cache_data.clear(); st.rerun()
    st.divider()
    with st.expander("📂 טעינת דוחות (PDF)"):
        st.file_uploader("גרור קובץ לכאן", type=['pdf'], key="u_v34")

# --- 4. DASHBOARD ---
if not df.empty:
    st.title(f"{sel_name} | {sel_q} 2025")
    if d['solvency_ratio'] < 150:
        st.markdown(f'<div style="background:#7a1a1c; border-right:5px solid #f85149; padding:10px; border-radius:6px; font-weight:700;">🚨 חריגת הון: סולבנסי ({d["solvency_ratio"]}%) נמוך מהיעד.</div>', unsafe_allow_html=True)

    k = st.columns(5)
    params = [
        ("סולבנסי", f"{int(d['solvency_ratio'])}%", r"\frac{OF}{SCR}", "חוסן הוני.", "יעד 150%."),
        ("יתרת CSM", f"₪{d['csm_total']}B", "CSM", "רווח עתידי גלום.", "מחסן רווחים."),
        ("ROE", f"{d['roe']}%", r"ROE = \frac{NI}{Eq}", "תשואה להון.", "איכות ניהול."),
        ("Combined", f"{d['combined_ratio']}%", "CR", "חיתום.", "יעילות."),
        ("NB Margin", f"{d['new_biz_margin']}%", "Margin", "רווחיות צמיחה.", "איכות מכירות.")
    ]
    for i in range(5):
        with k[i]: render_metric_card(*params[i])

    tabs = st.tabs(["📉 מגמות", "🏛️ סולבנסי II", "📑 מגזרים IFRS 17", "⛈️ Stress Test", "🏁 השוואה"])
    
    with tabs[0]: st.plotly_chart(px.line(c_df, x='quarter', y=['solvency_ratio', 'roe'], markers=True, template="plotly_dark", height=280), use_container_width=True)
    ...
    with tabs[1]:
        ...
        with cb: st.plotly_chart(px.pie(names=['שוק', 'חיתום', 'תפעול'], values=[d['mkt_risk'], d['und_risk'], d['operational_risk']], hole=0.6, template="plotly_dark", height=280, title="סיכוני SCR"), use_container_width=True)
    ...
    with tabs[2]:
        ...
        st.info(f"**Loss Component (LC):** ₪{d['loss_comp']}B - פוליסות בהפסד.")
    ...
    with tabs[4]:
        pm = st.selectbox("בחר מדד:", ['solvency_ratio', 'roe', 'combined_ratio', 'expense_ratio', 'csm_total'])
