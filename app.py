...
# --- 1. הגדרות מערכת ועיצוב EXECUTIVE (v68.0 VALIDATED) ---
...
def get_market_ticker():
    ...
    except: return "סנכרון מדדי בורסה בביצוע..."

# מנוע מבזקי רגולציה ושוק - סריקת עומק שבועית
...
m_ticker_html = get_market_ticker()
n_ticker_html = get_regulatory_news()

# CSS - תיקון הסטה צבעונית ו-Z-Index
st.markdown(f"""
    <style>
    ...
    .m-stripe {{
        background-color: #000000; padding: 12px 20px; border-bottom: 1px solid #334155;
        overflow: hidden; white-space: nowrap;
    }}
    
    .n-stripe {{
        background-color: #450a0a; padding: 7px 20px; border-bottom: 2px solid #7a1a1c;
        overflow: hidden; white-space: nowrap;
    }}
    
    .scroll-inner {{
        display: inline-block; padding-right: 100%; animation: tAnim 75s linear infinite;
        font-family: sans-serif; font-size: 0.92rem; color: #ffffff !important;
    }}
    @keyframes tAnim {{ 0% {{ transform: translateX(0); }} 100% {{ transform: translateX(-100%); }} }}
    
    .main-content-spacer {{ margin-top: 130px; }}

    /* Sidebar - חלון החיפוש (שכבה עליונה ביותר) */
    ...
    <div class="ticker-fixed-box">
        <div class="m-stripe"><div class="scroll-inner">{m_ticker_html} &nbsp;&nbsp;&nbsp;&nbsp; | &nbsp;&nbsp;&nbsp;&nbsp; {m_ticker_html}</div></div>
        <div class="n-stripe"><div class="scroll-inner">📢 מודיעין רגולטורי ופיננסי: {n_ticker_html} &nbsp;&nbsp;&nbsp;&nbsp; ● &nbsp;&nbsp;&nbsp;&nbsp; {n_ticker_html}</div></div>
    </div>
...
if not df.empty and d is not None:
    ...
    with tabs[0]:
        ...
        r_cols = st.columns(3)
        with r_cols[0]: render_kpi("Loss Ratio", f"{d['loss_ratio']}%", r"LR", "אימות חיתום.", "עלייה = סיכון.")
